    ...
    do {
        ...
        for(int i = 0; i < 10000; i++) {
            if(bots[i].vals[0] != 0 && bots[i].vals[1] != 0) {
                anything_to_do = true;
                int lower      = std::min(bots[i].vals[0], bots[i].vals[1]);
                int higher     = std::max(bots[i].vals[0], bots[i].vals[1]);
